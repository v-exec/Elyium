...
class Interface {
  ...
  private void idle() {
    ...
    //display idle image
    ASCII(50, height/2 - 250, source, true);
  }
  ...
  private void narrative(Entity object) {
    ...
    //render once
    background(0);
    ASCII(50, 50, source, object.animate);
    ...
    textAlign(LEFT);
    textSize(textSize);
    text(text, width/2, 50, width / 2 - 50, height - height/4);

    //display choices
    for (int i = 0; i < choices.length; i++) {
      if (choices[i] != "null") {
        ASCII(width/2, height/2 + (i * choiceSpacing), choiceIcon, true);

        rectMode(CORNER);
        textAlign(LEFT);
        textSize(textSize);

        text(choices[i], width/2 + 20, height/2 + (i * choiceSpacing), width - (width/10), 50);
      }
    }
  }
  ...
  private void menu() {
    ...
    text(mapper.latitude, 50, 50);
    text(mapper.longitude, 50, 50 + 40);
    ...
    for (int i = narrator.entityTick + 1; i < narrator.entities.length; i++) {
      ...
      ASCII(width/10 + ((width - width/5)/3 * col), 25 * row, entityIcon, true);

      rectMode(CORNER);
      textAlign(LEFT);
      textSize(textSize);

      text(narrator.entities[i].name, width/10 + ((width - width/5)/3 * col), 25 * row + 20);
      text(trim(str(narrator.entities[i].latitude)), width/10 + ((width - width/5)/3 * col), 25 * row + 40);
      text(trim(str(narrator.entities[i].longitude)), width/10 + ((width - width/5)/3 * col), 25 * row + 60);
      col++;
    }
    rendered = false;
  }
  ...
  private void control() {
    if (input) {
      //menu toggle
      if (mouseX >= width-100 && mouseX <= width && mouseY >= height-100 && mouseY <= height) menuToggle = !menuToggle;

      //choice selection
      if (choices[0] != "null" && choices[0] != null) {
        for (int i = 0; i < choices.length; i++) {
          if (choices[i] != "null") {
            if (mouseX >= width/2 && mouseX <= width - width/10 && mouseY >= height/2 + (i * choiceSpacing) && mouseY <= height/2 + choiceSpacing + (i * choiceSpacing)) {
              narrator.choose(i);
              refresh = true;
            }
          }
      ...
      if (inIdle && menuToggle) {
        ...
        for (int i = narrator.entityTick + 1; i < narrator.entities.length; i++) {
          ...
          if (mouseX >= width/10 + ((width - width/5)/3 * col) && mouseX <= width/10 + ((width - width/5)/3 * col) + 200 && mouseY >= 25 * row && mouseY <= 25 * row + 60) {
